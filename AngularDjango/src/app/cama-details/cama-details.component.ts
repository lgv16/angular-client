...
export class CamaDetailsComponent implements OnInit {
  ...
  updateActive(isActive: boolean) {
    this.camaService.updateCama(this.cama.id,
      { codigo: this.cama.codigo, name: this.cama.name, age: this.cama.age, 
        enfermedad: this.cama.enfermedad, estado: this.cama.estado,
        aislamiento: this.cama.aislamiento, active: isActive })
      .subscribe(
        data => {
          console.log(data);
